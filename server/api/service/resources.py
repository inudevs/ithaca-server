...
from server.api.question import service_api
from bson import ObjectId


@service_api.get('/<question_id:ObjectId>')
@doc.summary('대화 조회')
async def DialogueView(request, question_id):
    pass


@service_api.post('/<question_id:ObjectId>')
@doc.summary('대화 전송')
async def DialoguePost(request, question_id):
    pass


@service_api.delete('/<question_id:ObjectId>')
@doc.summary('대화 종료')
async def DialogueEnd(request, question_id):
    pass


@service_api.post('/request/teacher/<dialogue_id:ObjectId>')
@doc.summary('선생님 리뷰 요청')
async def RequestTeacher(request, question_id):
    pass


@service_api.post('/request/offline/<dialogue_id:ObjectId>')
